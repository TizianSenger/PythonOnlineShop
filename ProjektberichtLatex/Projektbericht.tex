...
% Wechsel zu arabischen Seitenzahlen und Mainmatter
\pagenumbering{arabic}
\setcounter{page}{1}
...
\begin{itemize}
    ...
    \item Passwörter werden mit kryptografischen Hash-Funktionen (Werkzeug: \texttt{werkzeug.security}) \cite{Flask} gespeichert
...
Nicht-funktionale Anforderungen beschreiben Qualitätsmerkmale und Betriebscharakteristiken (detailliert in \autoref{tab:nfa}):
...
Der Shop implementiert die folgenden Rechte (Details siehe \autoref{tab:dsgvo}):

\paragraph{Datenschutzrichtlinien im Shop:}

Die Authentifizierungsmechanismen sind detailliert in \autoref{tab:auth} dokumentiert.
...
PCI-DSS ist ein internationaler Standard zur Sicherung von Zahlungskartendaten \cite{PCI-DSS}. Der Shop erkennt an, dass die Verarbeitung von Zahlungsdaten eine hohe Verantwortung trägt.
...
Die PSD2-Richtlinie \cite{PSD2} regelt Zahlungsdienste in der EU und fordert unter anderem:
...
Der Shop wurde mit HTML, CSS und JavaScript implementiert. Die Template-Engine Jinja2 ermöglicht dynamische Inhalte. Im Folgenden sind die Hauptseiten des Shops beschrieben (siehe auch \autoref{fig:startseite} bis \autoref{fig:admin_products}):
...
Die Startseite ist in \autoref{fig:startseite} visualisiert.

\paragraph{Login und Registrierung:}

Benutzer müssen sich registrieren oder anmelden, um zu bestellen. Das Login-Interface ist in \autoref{fig:login} dargestellt:
...
Wenn ein Benutzer auf ein Produkt klickt, wird die Detailseite angezeigt (siehe \autoref{fig:produktdetail}):
...
Der Warenkorb-View zeigt alle hinzugefügten Artikel (siehe \autoref{fig:warenkorb}):
...
Das Checkout-Formular ist das Herzstück der Bestellabwicklung (siehe \autoref{fig:checkout}):
...
Nach erfolgreicher Zahlung wird eine Bestätigungsseite angezeigt (siehe \autoref{fig:bestaetigung}):
...
Angemeldete Benutzer können ihre Bestellungshistorie einsehen (siehe \autoref{fig:bestellungen}):
...
Administratoren haben Zugriff auf ein spezielles Interface (siehe \autoref{fig:admin_products}):
...
Das folgende ER-Diagramm visualisiert das Datenbankschema (siehe \autoref{fig:er_diagramm}):

\subsection{Datenbeschreibung}
\label{subsec:datenbeschreibung}

Folgende Tabellen werden im System verwendet (siehe \autoref{tab:user}, \autoref{tab:product}, \autoref{tab:order}, \autoref{tab:consent}, \autoref{tab:audit} im Anhang):

\paragraph{USER-Tabelle:}

Speichert Benutzerinformationen für Authentifizierung und Profilverwaltung (siehe \autoref{tab:user}).

\paragraph{PRODUCT-Tabelle:}

Speichert Produktinformationen für den Katalog (siehe \autoref{tab:product}).

\paragraph{ORDER-Tabelle:}

Speichert Bestellungen mit Metadaten (siehe \autoref{tab:order}).

\paragraph{CONSENT-Tabelle:}

Speichert Einwilligungen des Benutzers (DSGVO) (siehe \autoref{tab:consent}).

\paragraph{AUDIT\_LOG-Tabelle:}

Speichert kritische Operationen für Compliance und Sicherheit (siehe \autoref{tab:audit}).
...
Das Hybrid Backend folgt diesem Datenfluss (siehe \autoref{tab:hybrid}):
...
Das Projekt nutzt folgende externe Python-Packages (siehe \texttt{requirements.txt} und \autoref{tab:dependencies}):
...
Der Onlineshop folgt einer klaren, modularen Architektur mit Separation of Concerns. Die Architektur besteht aus vier Schichten (siehe \autoref{fig:schichten}):
...
Das Projekt folgt dem Prinzip Separation of Concerns (SoC), das bedeutet jedes Modul hat eine klare, einzelne Verantwortung. Dieses Architektur-Muster ist ein Best Practice in der Software-Entwicklung \\cite{Flask}:
...
Der Shop implementiert Schutzmaßnahmen gegen häufige Web-Angriffsvektoren \\cite{OWASP}:
...
Zielwerte für Produktionsumgebung sind Teil der nicht-funktionalen Anforderungen (siehe \autoref{tab:nfa}):
...
Alle beschriebenen Anforderungen aus Kapitel 1 wurden validiert (siehe \autoref{tab:ziele}):
...
Manuelle Tests in verschiedenen Browsern zeigten folgende Ergebnisse (siehe \autoref{tab:browser}):
...
Mehrere Aspekte des Projekts können als besonders erfolgreich bewertet werden (siehe auch \autoref{tab:erfolg}):

\subsection{Hybrid-Backend-Architektur}
\label{subsec:erfolg_hybrid}

Die Implementierung des Hybrid-Backends (SQLite mit CSV-Fallback, siehe \autoref{tab:hybrid}) war eine gelungene Design-Entscheidung:
...
\textbf{Vergleich:} Für MVP war raw SQL pragmatisch. Für Production würde SQLAlchemy + Alembic empfohlen.
...
Das Projekt erforderte systematische Planung in mehreren Phasen (siehe \autoref{tab:phasen}) mit insgesamt ca. 8 Wochen Entwicklungszeit für ein funktionsfähiges MVP mit Qualitätssicherung.
...
Alle in der Aufgabenstellung definierten Ziele wurden erreicht (siehe \autoref{tab:ziele}):
...
IU Internationale Hochschule GmbH (2024). \textit{Aufgabenstellung 2: Konzeption und Umsetzung eines einfachen Onlineshops}. Verfügbar unter: myCampus.

\bibitem{Prüfungsleitfaden} 
IU Internationale Hochschule GmbH (2024). \textit{Prüfungsleitfaden zur Erstellung eines Projektberichts}. Verfügbar unter: myCampus.

\bibitem{Flask} 
Ronacher, A. et al. (2024). \textit{Flask Web Development Framework -- Documentation}. Verfügbar unter: https://flask.palletsprojects.com/ [Zugegriffen: 04. Januar 2026].

\bibitem{Python} 
Python Software Foundation (2024). \textit{The Python Language Reference}. Verfügbar unter: https://docs.python.org/ [Zugegriffen: 04. Januar 2026].

\bibitem{SQLite} 
Hipp, D. R., Kennedy, D. und Wyrick, J. (2024). \textit{SQLite Database Engine -- Official Documentation}. Verfügbar unter: https://www.sqlite.org/docs.html [Zugegriffen: 04. Januar 2026].

\bibitem{DSGVO} 
Europäisches Parlament und Rat (2016). \textit{Verordnung (EU) 2016/679 des Europäischen Parlaments und des Rates vom 27. April 2016 zum Schutz natürlicher Personen bei der Verarbeitung personenbezogener Daten (Datenschutz-Grundverordnung -- DSGVO)}. \textit{Amtsblatt der Europäischen Union}, L119, S. 1-88.

\bibitem{PCI-DSS} 
Payment Card Industry Security Standards Council (2024). \textit{PCI Data Security Standard (PCI DSS) -- Version 4.0}. Verfügbar unter: https://www.pcisecuritystandards.org/ [Zugegriffen: 04. Januar 2026].

\bibitem{PSD2} 
Europäisches Parlament und Rat (2015). \textit{Richtlinie (EU) 2015/2366 vom 25. November 2015 über Zahlungsdienste im Binnenmarkt, zur Änderung der Richtlinien 2002/65/EG, 2009/110/EG und 2013/36/EU und zur Aufhebung der Richtlinie 2007/64/EG (PSD2)}. \textit{Amtsblatt der Europäischen Union}, L337, S. 35-127.

\bibitem{OWASP} 
OWASP Foundation (2024). \textit{OWASP Top 10 -- Web Application Security Risks}. Verfügbar unter: https://owasp.org/www-project-top-ten/ [Zugegriffen: 04. Januar 2026].

\bibitem{Werkzeug}
Ronacher, A. (2024). \textit{Werkzeug -- The Utility Library Behind Flask}. Verfügbar unter: https://werkzeug.palletsprojects.com/ [Zugegriffen: 04. Januar 2026].
